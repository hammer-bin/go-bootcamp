...
func main() {
	...
	title := []string{"First Name", "Last Name", "Nickname"}
	a := []string{"Albert", "Einstein", "time"}
	b := []string{"Isaac", "Newton", "apple"}
	c := []string{"Stephen", "Hawking", "blackhole"}
	d := []string{"Marie", "Curie", "radium"}
	e := []string{"Charles", "Darwin", "fittest"}

	names := [][]string{title, a, b, c, d, e}
